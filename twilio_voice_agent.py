...
try:
    ...
    from pipecat.frames.frames import (
        TextFrame, TTSTextFrame, LLMTextFrame, 
        AudioRawFrame, TranscriptionFrame, UserSpeakingFrame,
    ...
    def __init__(self):
        ...
        self.language_configs = {
            "es-LA": {
                ...
                "system_prompt": "Eres un agente de servicio al cliente útil, responde en español mexicano, maneja casos como reservas o soporte. Sé amigable, profesional y eficiente. Entiendes y usas expresiones mexicanas coloquiales cuando es apropiado."
            },
            "en-US": {
                "name": "English (US)",
                ...
                "system_prompt": "You are a helpful customer service agent, respond in English, handle cases like reservations or support. Be friendly, professional, and efficient."
            }
        }
    ...
    def update_language(self, detected_lang: str, confidence: float):
        ...
        if detected_lang != self.current_language and confidence >= self.language_switch_threshold:
            ...
            if self.consecutive_detections >= 2:
                ...
                logger.info(f"🔄 Language switched from {old_language} to {detected_lang} (confidence: {confidence:.2f})")
                return True
...
class PerformanceMonitor:
    ...
    def get_call_summary(self, call_sid: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_roundtrip_latency': statistics.mean(metrics['roundtrip_latencies']) if metrics['roundtrip_latencies'] else 0,
...
class TwilioVoiceAgent:
            ...
            class ConversationProcessor(FrameProcessor):
                ...
                async def process(self, frame):
                    ...
                    if isinstance(frame, UserStartedSpeakingFrame):
                        ...
                        if user_text and user_text != self.last_user_input:
                            ...
                            detected_lang, confidence = self.agent.language_manager.detect_language_from_text(user_text)
                    ...
                    if (self.silence_start and 
                        current_time - self.silence_start > self.voicemail_threshold and
                ...
                async def _get_ai_response(self, user_input: str):
                    ...
                    try:
                        ...
                        # Get LLM response
                        llm_start = time.time()
                        response = await self.agent.llm_service.complete(
                            messages=self.conversation_history
                        )
                        llm_latency = time.time() - llm_start
                        
                        if self.current_call_sid:
                            self.agent.performance_monitor.record_llm_latency(
                                self.current_call_sid, llm_latency
                            )
                        ...
                        if response and hasattr(response, 'content'):
                            ...
                            if self.current_call_sid:
                                self.agent.performance_monitor.record_tts_latency(
                                    self.current_call_sid, tts_latency
                                )
                            ...
                            if total_latency > self.agent.latency_target:
                                logger.warning(f"⚠️ Latency {total_latency:.3f}s exceeds target {self.agent.latency_target}s")
                            else:
                                logger.info(f"✅ Latency target met: {total_latency:.3f}s < {self.agent.latency_target}s")
    ...
    def generate_greeting_twiml(self) -> str:
        ...
        stream = ET.SubElement(connect, "Stream", url="/voice-stream")
...
def consent_response():
    ...
    try:
        ...
        consent_words = ['sí', 'si', 'yes', 'ok', 'vale', 'continuar', 'continue', 'proceed']
